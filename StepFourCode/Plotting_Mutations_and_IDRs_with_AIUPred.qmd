...
In this module, we will be graphing the frequency of cancer-associated, PTM-disrupting mutations along with predicted IDRs from AIUPred. The first file is called `Mutation_Freuency_Count.csv` and is the output of running `Step2Code.qmd`. If you have not completed that module, you may also just download this file from the `/output` directory of the GitHub repository. Then, replace the gene names in the cell below with the gene names that you would like to plot. The genes we have included in the example here are the genes that we included in some of our analyses.
