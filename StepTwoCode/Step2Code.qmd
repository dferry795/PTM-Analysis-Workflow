...
## First, read the csv files from your local environment. Edit this part and include the path to these files as downloaded on github
...
disruption = read_csv("~Path to file/PTM_Disrupting_Mutations.csv")

# EDIT HERE: Insert absolute path to files, and it selects the columns needed
frequent_mutations = read_tsv("~Path to file/TCGA_Frequent_Mutations.tsv") %>% 
...
ids = read_excel("~Path to file/idmap.xlsx") %>%
  rename(gene = symbol) %>%
...
*(You might get some warnings, but it's okay)*
...
You will need to write this data frame to a file so that you can use it later for other plots in Step 4. Here is the command you can run to write it to a file for later.

```{r}
write_csv(Joined_Data, "Mutation_Freuency_Count.csv")
```
...
Congrats! Now you ave joined the data frame to produce the most frequent (across all cancer types) PTM disrupting mutations, and selected those 6 proteins of most interest to us. You can now also see visually what the mutations looks like across the linear genome of that protein.
