...
In this module, we will be graphing the frequency of cancer-associated, PTM-disrupting mutations along with predicted IDRs from AIUPred. The first file is the output of running `Step2Code.qmd`. If you have not completed that module, you must do that first, because that data frame is the first input for this module. If you have already completed that module, then start by replacing the file path with the path to the .csv file containing the data frame from the `Step2Code.qmd` module. Then, replace the gene names with the gene names that you would like to plot. The genes we have included in the example here are the genes that we included in some of our analyses.
...
Mutation_PTM_Frequency = read_csv("~Path to file/Mutation_Freuency_Count.csv")
...
You will not be able to see the graphs here, but they have been downloaded and you can open them in your Downloads folder to view them.
